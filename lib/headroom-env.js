...
(function () {
  ...
  // The base URL Claude Code talks to when nothing else claims it. Exported
  // so callers can compare against it (e.g. to detect a stale/ambient value)
  // without hardcoding the literal.
  var DIRECT_ANTHROPIC_BASE_URL = 'https://api.anthropic.com';

  /**
   * Resolve who owns ANTHROPIC_BASE_URL for a column, in BOTH directions.
   *
   * Historically `buildHeadroomEnv` only had an opinion when Headroom was ON
   * — the disabled case set nothing, spawning "plainly". That was a latent
   * bug: with nothing set, Claude Code falls back to whatever is already in
   * scope — a leftover env var from the shell, or (worse) an `env` block in
   * `~/.claude/settings.json` / `<project>/.claude/settings*.json`, which
   * OVERRIDES the inherited process environment entirely (proven empirically
   * — see the module's callers/spec). So turning Headroom off could silently
   * leave a column routed through a proxy that no longer exists.
   *
   * The fix: the app's toggle always wins the base URL. When Headroom is
   * enabled this delegates to `buildHeadroomEnv` (same logic, not
   * duplicated). When it is disabled, this pins `ANTHROPIC_BASE_URL` to
   * `DIRECT_ANTHROPIC_BASE_URL` explicitly, so "off" is an assertion, not an
   * absence.
   *
   * `buildHeadroomEnv` itself is UNCHANGED and keeps its own null-on-disabled
   * contract — other callers (e.g. the renderer's context meter) depend on
   * that null meaning "no Headroom binding applies" and must not be handed a
   * literal base URL.
   *
   * @param {{ enabled?: any, hasEndpoint?: any, isClaude?: any, oneM?: any, oneMModel?: string, port?: number, hasMcp?: any }} input
   * @returns {object|null}
   */
  function resolveBaseUrlBinding(input) {
    input = input || {};
    // An arbitrary-command column isn't running Claude Code, so the app has
    // no opinion about its base URL either way.
    if (input.isClaude === false) return null;
    // A local-endpoint preset owns the base URL in both directions too — its
    // own env block (see getEndpointEnv in main.js) is the source of truth.
    if (input.hasEndpoint) return null;
    if (input.enabled) return buildHeadroomEnv(input);
    return { ANTHROPIC_BASE_URL: DIRECT_ANTHROPIC_BASE_URL };
  }

  // The full set of env keys resolveBaseUrlBinding/buildHeadroomEnv ever
  // inject — used to build the deterministic --settings JSON. This MUST
  // stay a hard-coded allowlist, not derived from whatever keys a caller
  // happens to pass: `--settings '{"env":{...}}'` sets env INSIDE Claude
  // Code, which bypasses pty-server.js's renderer-supplied-env blocklist
  // (see pty-server.js:143, which strips NODE_OPTIONS/NODE_PATH/LD_*/etc
  // from a spawn's env). If this function forwarded arbitrary keys, a
  // renderer-controlled env object would reopen that exact RCE class via
  // --settings instead of env — so only these three keys ever leave here.
  var INJECTED_SETTINGS_ENV_KEYS = ['ANTHROPIC_BASE_URL', 'ANTHROPIC_MODEL', 'ENABLE_TOOL_SEARCH'];

  /**
   * Find the LAST `--settings` occurrence in args (positional `--settings
   * <value>` or single-token `--settings=<value>`), ignoring a malformed
   * trailing bare `--settings` with no value. Returns null if none.
   * @param {string[]} args
   * @returns {{ index: number, isSingleToken: boolean, value: string }|null}
   */
  function findLastSettingsOccurrence(args) {
    var found = null;
    for (var i = 0; i < args.length; i++) {
      if (args[i] === '--settings' && i + 1 < args.length) {
        found = { index: i, isSingleToken: false, value: args[i + 1] };
      } else if (typeof args[i] === 'string' && /^--settings=/.test(args[i])) {
        found = { index: i, isSingleToken: true, value: args[i].slice('--settings='.length) };
      }
    }
    return found;
  }
  ...
  /**
   * Parse a `--settings` value as a JSON object (not array, not primitive).
   * Returns null for anything else, including a file path.
   * @param {string} value
   * @returns {object|null}
   */
  ...
  /**
   * The value of the last `--settings` in `args`, if it's a file path (i.e.
   * does NOT parse as JSON) — a shape `applyBaseUrlSettingsArg` cannot merge
   * into, because the renderer can't read the user's file to combine it.
   * Callers use this to know when the base-URL binding stood down for a
   * column, so they can surface that to the user.
   * @param {string[]} args
   * @returns {string|null}
   */
  function findUnmergeableSettingsFile(args) {
    args = Array.isArray(args) ? args : [];
    var last = findLastSettingsOccurrence(args);
    if (!last) return null;
    return parseSettingsObject(last.value) ? null : last.value;
  }

  /**
   * Make a base-URL binding authoritative by also passing it as
   * `--settings '{"env":{...}}'`, which outranks any `env` block in
   * `~/.claude/settings.json` / `<project>/.claude/settings*.json` — the
   * mechanism that made `resolveBaseUrlBinding`'s process-env binding alone
   * insufficient (see that function's doc-comment).
   *
   * `--settings` is LAST-WINS on the CLI, not deep-merged, so this function
   * MERGES our env into the last `--settings` in `args` in place — rather
   * than appending a second `--settings`, which would silently discard
   * whatever the last one carried (verified: an ultracode column's
   * `--settings {"ultracode":true,...}` was being wholesale discarded by a
   * naive append, silently turning ultracode off on every respawn):
   *
   *   - Last `--settings` parses as a JSON OBJECT (ours, the user's own, or
   *     an ultracode-style settings blob): every existing top-level key is
   *     preserved, and the three keys in `INJECTED_SETTINGS_ENV_KEYS` are
   *     FULLY REPLACED inside `env` (deleted, then the new ordered set
   *     applied) rather than overlaid — a re-stamp that carries fewer of
   *     the three (e.g. the direct binding, which sets only
   *     ANTHROPIC_BASE_URL) must not leave a stale ANTHROPIC_MODEL or
   *     ENABLE_TOOL_SEARCH surviving from an earlier proxy stamp. Those
   *     three keys are app-owned inside the merged block: if the user's
   *     own `--settings` sets e.g. ANTHROPIC_MODEL, the app's binding
   *     replaces it too — the same "the app's toggle always wins"
   *     precedence `resolveBaseUrlBinding` applies to the base URL,
   *     applied consistently to the whole binding. Every other `env` key
   *     the user set, and every non-`env` top-level key, survives
   *     untouched. The token is replaced in place (same position, still
   *     exactly one `--settings`).
   *   - Last `--settings` does NOT parse as JSON (a file path): unmergeable
   *     — we can't read the user's file from here to combine it, and
   *     appending a second `--settings` after it would silently neutralise
   *     it (last-wins), dropping the user's permissions/hooks/model config
   *     with no error. So we stand down entirely and return `args`
   *     unchanged; the column falls back to process-env only for that
   *     column. `findUnmergeableSettingsFile` lets callers detect this and
   *     warn the user the binding isn't authoritative there.
   *   - No `--settings` at all: append a fresh `--settings '{"env":{...}}'`.
   *
   * A malformed trailing bare `--settings` with no value is always dropped
   * first (not a valid invocation either way).
   *
   * When `env` is null/undefined/empty, this is a no-op: returns a copy of
   * `args` untouched (nothing appended, nothing merged, nothing stripped —
   * merge-in-place needs nothing undone, unlike the old strip-then-append
   * approach).
   *
   * Key order inside `env` is fixed (INJECTED_SETTINGS_ENV_KEYS) for OUR
   * keys, appended after any of the user's own `env` keys, so a respawn
   * with the same env produces byte-identical args (idempotent by
   * construction: merging our own already-merged output back in yields the
   * same JSON).
   *
   * @param {string[]} args
   ...
   * @returns {string[]}
   */
  function applyBaseUrlSettingsArg(args, env) {
    args = Array.isArray(args) ? args : [];
    // Drop a malformed trailing bare --settings (no value) — invalid input
    // either way, and it would otherwise get misread as the value of a
    // freshly-appended --settings below.
    var trimmed = (args.length && args[args.length - 1] === '--settings')
      ? args.slice(0, -1)
      : args.slice();

    if (!env || Object.keys(env).length === 0) return trimmed;

    var orderedEnv = {};
    INJECTED_SETTINGS_ENV_KEYS.forEach(function (k) {
      if (k in env) orderedEnv[k] = env[k];
    });

    var last = findLastSettingsOccurrence(trimmed);
    var lastObj = last ? parseSettingsObject(last.value) : null;

    if (last && !lastObj) {
      // Unmergeable file-path --settings — the user's file wins. Stand down.
      return trimmed;
    }

    if (last && lastObj) {
      var merged = Object.assign({}, lastObj);
      // The three app-owned keys are fully replaced, not overlaid: an env
      // block stamped with the proxy binding (BASE_URL+MODEL+TOOL_SEARCH)
      // and then re-stamped with the direct binding (BASE_URL only) must
      // not leak MODEL/TOOL_SEARCH forward from the earlier stamp — those
      // are stale app state, not user config. So we strip all three from
      // whatever's already there before overlaying the new ordered set;
      // anything else the user put in `env` survives untouched. A malformed
      // `env` in the user's own JSON (e.g. "env": "x" or "env": []) isn't a
      // plain object — Object.assign on it would spray index-keyed junk
      // ("0": "x", ...) into the emitted settings, so fall back to {} instead
      // of trusting its shape.
      var rawEnv = lastObj.env;
      var isPlainEnv = !!rawEnv && typeof rawEnv === 'object' && !Array.isArray(rawEnv);
      var keptEnv = Object.assign({}, isPlainEnv ? rawEnv : {});
      INJECTED_SETTINGS_ENV_KEYS.forEach(function (k) { delete keptEnv[k]; });
      merged.env = Object.assign({}, keptEnv, orderedEnv);
      var newValue = JSON.stringify(merged);
      var out = trimmed.slice();
      out[last.isSingleToken ? last.index : last.index + 1] =
        last.isSingleToken ? ('--settings=' + newValue) : newValue;
      return out;
    }

    // No existing --settings at all — append a fresh one.
    var out2 = trimmed.slice();
    out2.push('--settings', JSON.stringify({ env: orderedEnv }));
    return out2;
  }
  ...
  var api = {
    ...
    resolveBaseUrlBinding: resolveBaseUrlBinding,
    applyBaseUrlSettingsArg: applyBaseUrlSettingsArg,
    findUnmergeableSettingsFile: findUnmergeableSettingsFile,
    DIRECT_ANTHROPIC_BASE_URL: DIRECT_ANTHROPIC_BASE_URL
  };
