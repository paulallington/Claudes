...
(function () {
   ...
   *     preserved, and `env` becomes `{...existing.env, ...ourEnv}` — our
   *     keys win inside `env`, the user's other `env` keys and every
   *     non-`env` key survive. The token is replaced in place (same
   *     position, still exactly one `--settings`).
  ...
  function applyBaseUrlSettingsArg(args, env) {
    ...
    if (last && lastObj) {
      ...
      merged.env = Object.assign({}, lastObj.env, orderedEnv);
