...
(function () {
  ...
  // inject — used both to build the deterministic --settings JSON and to
  // recognise (vs. a user's own --settings) an injected pair on respawn.
  var INJECTED_SETTINGS_ENV_KEYS = ['ANTHROPIC_BASE_URL', 'ANTHROPIC_MODEL', 'ENABLE_TOOL_SEARCH'];

  /**
   * Whether a JSON-parsed `--settings` value is one WE injected (as opposed
   * to the user's own `--settings {...}` inline JSON): exactly one top-level
   * key, `env`, whose keys are all drawn from INJECTED_SETTINGS_ENV_KEYS.
   * @param {string} value
   * @returns {boolean}
   */
  function isInjectedSettingsValue(value) {
    var parsed;
    try { parsed = JSON.parse(value); } catch (e) { return false; }
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return false;
    var topKeys = Object.keys(parsed);
    if (topKeys.length !== 1 || topKeys[0] !== 'env') return false;
    var env = parsed.env;
    if (!env || typeof env !== 'object' || Array.isArray(env)) return false;
    return Object.keys(env).every(function (k) { return INJECTED_SETTINGS_ENV_KEYS.indexOf(k) !== -1; });
  }
   ...
   *
   * Always strips any PREVIOUSLY-INJECTED `--settings` pair first (both the
   * positional `--settings <value>` and single-token `--settings=<value>`
   * shapes), recognised via `isInjectedSettingsValue` — this mirrors
   * `reconcileModelArgForRespawn` in this same file: respawn rebuilds args
   * from stored `cmdArgs`, so without the strip a column would accumulate a
   * stale binding on every respawn. A `--settings` pointing at a file path,
   * or carrying any other JSON, is the USER's and is left untouched, in
   * place. A malformed trailing bare `--settings` with no value is always
   * dropped (not a valid invocation either way).
   *
   * When `env` is null/undefined/empty, this only strips — nothing is
   * appended, so a column with no binding (e.g. isClaude === false) never
   * gets an empty `--settings '{"env":{}}'`.
   *
   * Key order inside the JSON is fixed (INJECTED_SETTINGS_ENV_KEYS) so a
   * respawn produces byte-identical args.
   *
   * @param {string[]} args
  ...
  function applyBaseUrlSettingsArg(args, env) {
    ...
    for (var i = 0; i < args.length; i++) {
      if (args[i] === '--settings') {
        if (i + 1 < args.length) {
          if (!isInjectedSettingsValue(args[i + 1])) out.push(args[i], args[i + 1]);
          i++;
        }
        // else: malformed trailing bare --settings with no value — dropped.
        continue;
      }
      if (typeof args[i] === 'string' && /^--settings=/.test(args[i])) {
        var tokenValue = args[i].slice('--settings='.length);
        if (!isInjectedSettingsValue(tokenValue)) out.push(args[i]);
        continue;
      }
      out.push(args[i]);
    }
    if (!env || Object.keys(env).length === 0) return out;
    var orderedEnv = {};
    INJECTED_SETTINGS_ENV_KEYS.forEach(function (k) {
      if (k in env) orderedEnv[k] = env[k];
    });
    out.push('--settings', JSON.stringify({ env: orderedEnv }));
    return out;
  }
