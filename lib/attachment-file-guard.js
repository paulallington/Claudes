...
(function () {
   ...
   * @returns {boolean}
   */
  function isAllowedOpenExt(filePath) {
    if (typeof filePath !== 'string' || !filePath) return false;
    return OPEN_ALLOWED_EXTS.indexOf(extOf(filePath)) !== -1;
  }
  ...
  function checkAttachmentOpenPath(candidate, roots, platform, deps) {
    if (typeof candidate === 'string' && candidate && !isAllowedOpenExt(candidate)) {
      return { ok: false, error: 'unsupported extension' };
    }
    ...
    if (!isAllowedOpenExt(result.path)) return { ok: false, error: 'unsupported extension' };
