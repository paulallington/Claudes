  ...
  var findings = base.findings.map(function (f) {
    ...
    return Object.assign({}, f, {
      resolution: {
        ...
        resolvedAt: nowIso,
        // Stamped once the answer has actually been handed to the automation
        // (see markResolutionsDelivered). Until then the resolution is
        // pending delivery and must not be pruned away.
        deliveredAt: null,
      },
    });
  });
...
/**
 * Stamp `resolution.deliveredAt` on the given finding ids — called once the
 * resolutions block built from them has been injected into an automation's
 * prompt, so the same answer is never replayed on a later run.
 *
 * Delivery is at-most-once by design: the stamp lands at spawn, so a run
 * that dies before reading its prompt does not get the answer re-offered.
 * The alternative (stamp on successful completion) would replay every
 * answer on any crashed run, which is the worse failure — an automation
 * re-actioning decisions it already carried out.
 *
 * Only findings that carry an as-yet-undelivered resolution are touched.
 ...
 * @returns {{ version: 1, findings: Array }}
 */
function markResolutionsDelivered(store, ids, now) {
  var nowIso = toIso(now);
  var base = normalizeStore(store);
  var wanted = Array.isArray(ids) ? ids : [];
  if (!wanted.length) return base;
  var findings = base.findings.map(function (f) {
    if (wanted.indexOf(f.id) === -1) return f;
    if (!f.resolution || f.resolution.deliveredAt) return f;
    return Object.assign({}, f, {
      resolution: Object.assign({}, f.resolution, { deliveredAt: nowIso }),
    });
  });
  return { version: 1, findings: findings };
}
...
/**
 * When a finding stopped needing the user's attention: either they dismissed
 * it (`acknowledgedAt`) or they answered its decision and that answer has
 * been delivered to the automation (`resolution.deliveredAt`). Findings with
 * neither are never pruned. Returns null when the finding is still live.
 * @param {{acknowledgedAt?: string|null, resolution?: {deliveredAt?: string|null}|null}} f
 * @returns {string|null}
 */
function settledAt(f) {
  if (!f) return null;
  if (f.acknowledgedAt) return f.acknowledgedAt;
  if (f.resolution && f.resolution.deliveredAt) return f.resolution.deliveredAt;
  return null;
}

/**
 * Evict from `list` down to `cap` entries, oldest-settled first (by
 * `settledAt` ascending, tie-broken by `lastSeenAt` ascending). Unsettled
 * findings — neither acknowledged nor answered-and-delivered — are never
 * eviction candidates, so `list` can remain over `cap` when there aren't
 * enough settled entries to evict. Findings with an unresolved decision are
 * likewise never candidates, even if acknowledged: an unanswered question
 * must stay collectable from the inbox.
 * @param {Array} list
 * @param {number} cap
 * @returns {Array}
 */
function evictOldestAcknowledgedFirst(list, cap) {
  ...
  while (kept.length > cap) {
    ...
    if (candidateIdx === -1) break; // nothing left to evict — all remaining are unsettled
...
/**
 * Prune `store`: drop settled findings older than `ackMaxAgeDays` (by
 * `lastSeenAt`), then enforce per-automation and global caps, evicting
 * oldest-settled first and then oldest `lastSeenAt`. Unsettled findings —
 * neither acknowledged nor answered-and-delivered — are never pruned, even
 * over cap; findings with an unresolved decision are protected the same
 * way, acknowledged or not. Pure — returns a new store.
 * @param {{ version: 1, findings: Array }} store
 * @param {string|Date} now
...
var api = {
  ...
  markResolutionsDelivered: markResolutionsDelivered,
