...
(function () {
  ...
  function applyPosition() {
    ...
    const p = loadPos();
    if (p) {
      const clamped = window.ClawdPosition.clampPosition(p, {
        innerWidth: window.innerWidth,
        innerHeight: window.innerHeight,
      });
      widgetEl.style.right = clamped.right + 'px';
      widgetEl.style.bottom = clamped.bottom + 'px';
      // Self-heal: if the saved position was off-screen, persist the correction.
      if (clamped.right !== p.right || clamped.bottom !== p.bottom) savePos(clamped);
    }
  }
