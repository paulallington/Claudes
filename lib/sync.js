...
// claudeRoot is the directory that CONTAINS `projects/` — a profile's
// configDir or `<home>/.claude`. Callers own the `.claude` join.
function localClaudeProjectDir(claudeRoot, projectPath) {
...
function startExportWatcher(opts) {
  ...
  // TODO(profiles): sync is project-scoped today and always mirrors Primary's
  // transcripts; thread the project's resolved profile root here.
  const src = localClaudeProjectDir(path.join(homedir, '.claude'), projectPath);
...
function startImportWatcher(opts) {
  ...
  // TODO(profiles): see startExportWatcher above.
  const dst = localClaudeProjectDir(path.join(homedir, '.claude'), projectPath);
