 ...
 * missing or empty.
 * @param {string} claudeRoot the directory that CONTAINS `projects/` — either
 *   a profile's configDir or `<home>/.claude`. Callers own the `.claude` join;
 *   this function does not add it.
...
/**
 * True when `p` resolves to a path inside (or equal to) the projects root
 * under `claudeRoot`. Used to reject any transcript candidate that would
 * escape `<claudeRoot>/projects` via traversal or an absolute out-of-root
 * path — including a transcript that belongs to a DIFFERENT profile's root,
 * which is the silent-failure case this whole module exists to prevent.
 ...
 *   2. The cwd-derived path (`columnTranscriptPath(root, cwd, sessionId)`).
 ...
 *
 * @param {object} opts
 * @param {string} opts.claudeRoot the directory that CONTAINS `projects/` —
 *   a profile's configDir or `<home>/.claude`. A transcript rooted under a
 *   DIFFERENT profile's root is rejected here, not just not-found: that is
 *   what keeps a wrong-root lookup a loud "no" instead of a silent one.
...
function resolveTranscriptPath(opts) {
  ...
  const root = o.claudeRoot;
  const triedCwdPath = columnTranscriptPath(root, o.cwd, o.sessionId);
  const triedProjectPath = columnTranscriptPath(root, o.projectKey, o.sessionId);
  ...
  const accepts = (p) =>
    typeof p === 'string' && p &&
    p.endsWith('.jsonl') &&
    root && isUnderProjectsRoot(root, p) &&
    exists(p);
