...
class CodexAppServerService {
  constructor(options) {
    ...
    this.onClaimExpired = typeof options.onClaimExpired === 'function' ? options.onClaimExpired : function () {};
    ...
    this.cancelSchedule = typeof options.clearTimeout === 'function' ? options.clearTimeout : clearTimeout;
  ...
  handleNotification(message) {
    ...
    if (message && message.method === 'thread/started') {
      ...
      this.pruneFreshClaims();
      if (typeof thread.cwd === 'string' && thread.cwd) {
        var cwd = normalizeWorkingDirectory(thread.cwd, this.platform);
        var claimIndex = this.freshClaims.findIndex((claim) => claim.cwd === cwd && !claim.adoptingThreadId);
        if (claimIndex !== -1) {
          var claim = this.freshClaims[claimIndex];
          claim.adoptingThreadId = thread.id;
          if (claim.timer) this.cancelSchedule(claim.timer);
          claim.timer = null;
          var claimedState = this.threadStates.get(thread.id) || createThreadState(thread.id);
          claimedState = reduceThreadNotification(claimedState, {
            method: 'thread/status/changed',
            params: { threadId: thread.id, status: thread.status }
          });
          this.threadStates.set(thread.id, claimedState);
          return this.adoptFreshThread(claim, thread);
        }
      }
      ...
      if (this.freshClaims.some((claim) => claim.adoptingThreadId === thread.id)) return;
    ...
    if (this.freshClaims.some((claim) => claim.adoptingThreadId === params.threadId)) return;
    this.onState(this.getThreadState(params.threadId));
  }

  async adoptFreshThread(claim, thread) {
    try {
      var result = await this.client.request('thread/resume', { threadId: thread.id });
      var claimIndex = this.freshClaims.indexOf(claim);
      if (claimIndex === -1) return;
      if (!result || !result.thread || result.thread.id !== thread.id) {
        throw new Error('Codex app-server resumed an unexpected thread');
      }
      this.seedThreadState(result, false);
      this.settleFreshClaim(claimIndex, false);
  ...
  seedThreadState(result, publish) {
    ...
    if (publish !== false) this.onState(this.getThreadState(thread.id));
    return thread.id;
  }
  ...
  async prepareFreshThread(cwd) {
    var normalizedCwd = normalizeWorkingDirectory(cwd, this.platform);
    while (true) {
      if (this.stopping || this.unavailable) throw new Error('Codex bridge unavailable until app restart');
      if (this.blockedFreshCwds.has(normalizedCwd)) {
        throw new Error('A Codex attach timed out for this project; restart Claudes before retrying');
      }
      this.pruneFreshClaims();
      var prior = this.freshClaims.find((claim) => claim.cwd === normalizedCwd);
      if (!prior) break;
      await prior.settled;
    }
    var now = this.now();
    ...
    this.freshClaims.push(claim);
    claim.timer = this.schedule(() => {
      var index = this.freshClaims.indexOf(claim);
      if (index !== -1 && !claim.adoptingThreadId) this.settleFreshClaim(index, true);
    }, this.claimTtlMs);
    if (claim.timer && claim.timer.unref) claim.timer.unref();
    return {
      mode: 'fresh',
      claimId: claimId,
      remoteUrl: this.remoteUrl,
  ...
  settleFreshClaim(index, expired) {
    var claim = this.freshClaims.splice(index, 1)[0];
    if (!claim) return null;
    if (claim.timer) this.cancelSchedule(claim.timer);
    claim.settle();
    if (expired) {
      this.blockedFreshCwds.add(claim.cwd);
      this.onClaimExpired({ claimId: claim.claimId, reason: 'timeout' });
    }
    return claim;
  }
  ...
  pruneFreshClaims() {
    var now = this.now();
    for (var i = this.freshClaims.length - 1; i >= 0; i--) {
      if (!this.freshClaims[i].adoptingThreadId && this.freshClaims[i].expiresAt <= now) {
        this.settleFreshClaim(i, true);
      }
    }
  }

  releaseFreshClaims(notify) {
    while (this.freshClaims.length) {
      var claim = this.settleFreshClaim(this.freshClaims.length - 1, false);
      if (notify) this.onClaimExpired({ claimId: claim.claimId, reason: 'unavailable' });
    }
  }

  isPreparationActive(prepared) {
    if (this.stopping || this.unavailable || !prepared || prepared.remoteUrl !== this.remoteUrl) return false;
    if (prepared.mode === 'fresh') {
      return this.freshClaims.some((claim) => claim.claimId === prepared.claimId);
    }
    return prepared.mode === 'resume' && isUuid(prepared.threadId);
  }
