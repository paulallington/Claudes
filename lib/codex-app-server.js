...
class CodexAppServerService {
  ...
  async prepareThread(options) {
    ...
    if (options.threadId !== undefined && options.threadId !== null) {
      ...
      var read = await client.request('thread/read', { threadId: options.threadId, includeTurns: false });
  ...
  markUnavailable(childAlreadyExited) {
    ...
    if (!childAlreadyExited && child && !child.killed) { try { child.kill(); } catch { /* already exited */ } }
    this.onUnavailable();
  }
