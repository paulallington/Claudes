...
class CodexAppServerService {
  ...
  async adoptFreshThread(claim, thread) {
    try {
      var result = await this.client.request('thread/resume', { threadId: thread.id, cwd: thread.cwd });
  ...
  async prepareFreshThread(cwd) {
    ...
    claim.timer = this.schedule(() => {
      ...
      if (index !== -1) this.settleFreshClaim(index, true);
    }, this.claimTtlMs);
  ...
  pruneFreshClaims() {
    ...
    for (var i = this.freshClaims.length - 1; i >= 0; i--) {
      if (this.freshClaims[i].expiresAt <= now) this.settleFreshClaim(i, true);
    }
  }
