...
/**
 * Resolve the working directory to use for locating/resuming a column's Claude
 * session transcript. For an 'auto-worktree' binding, col.cwd is only a Git-tab
 * hint (the process ran at the project root and its transcript lives under the
 * root key), so we must use projectRoot. Manual/unset bindings use their own cwd.
 *
 * @param {{ cwd?: string|null, cwdSource?: string|null }|null|undefined} entry
 * @param {string} projectRoot
 * @returns {string}
 */
function resolveSessionLookupCwd(entry, projectRoot) {
  if (entry && entry.cwdSource === 'auto-worktree') return projectRoot;
  return (entry && entry.cwd) || projectRoot;
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = { isUsableSessionTarget, shouldBindHookSession, resolveInputColumn, resolveSessionLookupCwd };
}
if (typeof window !== 'undefined') {
  window.SessionTarget = { isUsableSessionTarget, shouldBindHookSession, resolveInputColumn, resolveSessionLookupCwd };
}
