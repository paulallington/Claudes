...
(function () {
  ...
  var MODELS = [
    ...
    pinned('claude-opus-5-5', 'Opus 5.5', 'opus', 1000000, 4.0, 20.0),
  ...
  // Fallbacks for an id that names a family but no known pinned version. NOT
  // applied to ids in MODELS.
  //
  // Every *older* model is pinned above, so an unpinned id is almost always a
  // model NEWER than this catalogue — the failure mode staleness actually
  // produces. Prices therefore track the current generation: guessing legacy
  // rates for a new model overstated cost 3x, which is the bug this module was
  // written to kill.
  //
  // Context deliberately does NOT follow the same logic and stays conservative.
  // The two errors are not symmetric: an over-large window means the meter
  // never warns and the user is ambushed by autocompact, while an over-small
  // one only warns early. Fail toward the annoying error, not the silent one.
  var FAMILY_FALLBACK = {
    opus: { input: 5.0, output: 25.0, contextWindow: 200000 },
