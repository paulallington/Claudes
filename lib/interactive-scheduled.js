...
function buildInteractiveArgs(opts) {
  ...
  // Mirrors spawnHeadlessClaude's guard in main.js: --session-id and
  // --resume are mutually exclusive to the CLI, so an automation whose own
  // extraArgs already carry either must not also get our minted
  // --session-id pushed in front of them.
  const hasConflictingSessionFlag = extraArgs.indexOf('--resume') !== -1 || extraArgs.indexOf('--session-id') !== -1;
...
// De-noise interactive-TUI output. An interactive `claude` session redraws its
// TUI constantly — truecolor SGR codes, cursor moves, and a status/spinner line
// ("thinking with xhigh effort", token counters, box-drawing) repainted every
// frame. Even with ANSI stripped, that spam concatenates into a huge, useless
// blob that bloats run records and slows the detail view. This strips the ANSI
// and removes the recurring TUI chrome, leaving the model's actual narration.
// Safe to run per-chunk (live streaming) or on the whole buffer (saved record);
// returns '' when a chunk is pure noise so callers can drop it.
var TUI_NOISE = [
  /thinking with [\w-]+ effort/gi,
  /thought for \d+\s*s?/gi,
  /(?:Baking|Flamb[ée]ing|Cooking|Simmering|Bootstrapping|Percolating|Marinating|Reticulating|Conjuring|Whisking|Distilling|Brewing|Noodling|Pondering|Cooked|Baked)[….]*/gi,
  /(?:↓|↑)?\s*\d[\d,]*\s*tokens?/gi,
  /esc to interrupt/gi,
  ...
  /running stop hook[^\n]*/gi,
  /ctrl\+g to edit[^\n]*/gi,
  /\bxhigh\b\s*[··]?\s*\/effort/gi
];
// Box-drawing, block elements, geometric shapes, misc symbols, dingbats
// (spinner glyphs like ✻ ✽ ✶ live here), braille, and misc technical arrows —
// all decorative TUI chrome with no textual value.
var GLYPH_RE = /[─-➿⬀-⯿⠀-⣿⏰-⏿■-◿•◉❯↓↑]/g;
function denoiseInteractive(text) {
  var s = stripAnsi(text);
  if (!s) return '';
  for (var i = 0; i < TUI_NOISE.length; i++) s = s.replace(TUI_NOISE[i], '');
  s = s.replace(GLYPH_RE, '');
  s = s.replace(/[ \t]{2,}/g, ' ').replace(/ *\n */g, '\n').replace(/\n{3,}/g, '\n\n');
  return s.trim();
}

// Keep at most maxLen chars, preferring the tail (the end of a run is the useful
// part) and marking the elision.
function capTail(s, maxLen) {
  s = String(s == null ? '' : s);
  maxLen = maxLen || 8000;
