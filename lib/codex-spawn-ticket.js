...
function createSpawnTicketStore(options) {
  ...
  function issue(binding, platform) {
    ...
    if (!bearer || !THREAD_RE.test(binding && binding.threadId || '') || !cwd || binding.remoteUrl !== remoteUrl) {
      throw new Error('invalid Codex spawn authorization');
    }
    const ticket = randomBytes(32).toString('hex');
    const expiresAt = now() + ttlMs;
    tickets.set(ticket, { threadId: binding.threadId, cwd, remoteUrl, expiresAt });
  ...
  function consume(binding, platform) {
    ...
    if (!cwd || stored.threadId !== binding.threadId || stored.cwd !== cwd || stored.remoteUrl !== binding.remoteUrl) return null;
    return bearer;
  }
...
module.exports = { createSpawnTicketStore, normalizeCwd, THREAD_RE, REMOTE_RE };
