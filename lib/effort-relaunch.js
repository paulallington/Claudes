...
(function () {
  ...
  function isStrippedFlag(flag) {
    // --session-id is stripped (with its uuid value) for defense-in-depth: a
    // fresh-spawn pin can leak into prior args, and resuming an existing session
    // via --session-id (vs --resume) is a CLI conflict.
