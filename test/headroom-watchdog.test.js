...
test('disabled -> idle and clears any accrued failures', () => {
  const r = planHeadroomWatchdog(S({ consecutiveFailures: 2 }), { enabled: false, installed: true, healthy: false, now: 1000 });
  assert.strictEqual(r.action, 'idle');
  assert.strictEqual(r.consecutiveFailures, 0);
});

test('not installed -> idle (nothing to supervise)', () => {
  const r = planHeadroomWatchdog(S({ consecutiveFailures: 5 }), { enabled: true, installed: false, healthy: false, now: 1000 });
  assert.strictEqual(r.action, 'idle');
  assert.strictEqual(r.consecutiveFailures, 0);
});

test('healthy probe -> idle and resets the failure streak', () => {
  const r = planHeadroomWatchdog(S({ consecutiveFailures: 2 }), { enabled: true, installed: true, healthy: true, now: 1000 });
  assert.strictEqual(r.action, 'idle');
  assert.strictEqual(r.consecutiveFailures, 0);
});

test('first unhealthy probe -> watch, not restart (tolerate a transient blip)', () => {
  const r = planHeadroomWatchdog(S(), { enabled: true, installed: true, healthy: false, now: 1000 });
...
test('below threshold stays watching', () => {
  ...
  for (let i = 1; i < FAILURE_THRESHOLD; i++) {
    const r = planHeadroomWatchdog(s, { enabled: true, installed: true, healthy: false, now: 1000 + i });
    assert.strictEqual(r.action, 'watch');
...
test('reaching the threshold triggers a restart, resets the streak, records a timestamp', () => {
  ...
  let r;
  for (let i = 1; i <= FAILURE_THRESHOLD; i++) {
    r = planHeadroomWatchdog(s, { enabled: true, installed: true, healthy: false, now: 5000 + i });
    s = r;
  }
...
test('a healthy probe after failures clears the streak so it must re-accrue', () => {
  ...
  s = planHeadroomWatchdog(s, { enabled: true, installed: true, healthy: true, now: 2000 });
  assert.strictEqual(s.consecutiveFailures, 0);
  const r = planHeadroomWatchdog(s, { enabled: true, installed: true, healthy: false, now: 2001 });
  assert.strictEqual(r.action, 'watch');
});
...
test('crash-loop guard: gives up after MAX_RESTARTS_IN_WINDOW restarts in the window', () => {
  ...
  const r = planHeadroomWatchdog(S({ consecutiveFailures: FAILURE_THRESHOLD - 1, restartTimestamps: prior }),
    { enabled: true, installed: true, healthy: false, now });
...
test('restarts older than the window are pruned and do not count toward the cap', () => {
  ...
  const r = planHeadroomWatchdog(S({ consecutiveFailures: FAILURE_THRESHOLD - 1, restartTimestamps: stale }),
    { enabled: true, installed: true, healthy: false, now });
  assert.strictEqual(r.action, 'restart');
...
test('backoff grows with the number of recent restarts', () => {
  const now = 200000;
  const one = planHeadroomWatchdog(S({ consecutiveFailures: FAILURE_THRESHOLD - 1, restartTimestamps: [now - 10] }),
    { enabled: true, installed: true, healthy: false, now });
